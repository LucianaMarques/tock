...
impl Trng {
    ...
    pub fn enable(&self) {
        ...
        // Issue a SW reset
        registers.sw_reset.write(SoftwareReset::RESET::SET);
        while registers.sw_reset.is_set(SoftwareReset::RESET) {}

        // Set the startup samples
        registers.ctl.modify(Control::STARTUP_CYCLES.val(1));
        ...
        registers.alarm_ctl.write(AlarmControl::ALARM_THR.val(0xFF));

        // Enable the TRNG
        registers.ctl.modify(Control::TRNG_EN::SET);
    }

    pub fn read_number_blocking(&self) -> u64 {
        let registers = &*self.registers;

        if !registers.ctl.is_set(Control::TRNG_EN) {
            self.enable();
        }

        // Wait for a number to be ready
        while !registers.irq_flag_stat.is_set(IrqStatus::READY) {}

        // Initiate generation of a new number
        registers.irq_flag_clr.write(IrqFlagClear::READY::SET);

        ((registers.out0.get() as u64) << 32) | (registers.out1.get() as u64)
    }
